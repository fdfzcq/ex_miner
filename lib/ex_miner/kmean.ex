defmodule ExMiner.Kmean do
  ...
  defp init_centroids(st, true) do
    ...
    |> Enum.reduce(st, fn point, state ->
      ...
      case state.dist < dis do
        true -> %{state | dist: dis, centroids: {point, p2}, group_a: [point], group_b: [p2]}
        false -> state
      end
    end)
  end
  ...
  defp cluster(st) do
    ...
    case result.centroids == st.centroids do
      true ->
        IO.puts("Get final centroids #{inspect(result.centroids)}")
        result

      _false ->
        new_st = %{result | cluster: cluster}
  ...
  defp remove_centroids_from_cluster(state) do
    new_cluster =
      state.cluster
      |> Enum.filter(fn p ->
        !Enum.any?(state.group_a, &(&1 == p)) && !Enum.any?(state.group_b, &(&1 == p))
      end)
  ...
  defp do_cluster(
         state = %{cluster: [h | t], centroids: {p1, p2}, group_a: group_a, group_b: group_b}
       ) do
    ...
    new_state =
      case dist_a < dist_b do
        true ->
          %{state | cluster: t, group_a: [h | group_a], group_b: Enum.filter(group_b, &(&1 != h))}

        false ->
          %{state | cluster: t, group_b: [h | group_b], group_a: Enum.filter(group_a, &(&1 != h))}
      end
